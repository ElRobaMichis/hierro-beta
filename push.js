...
function pushSettings(){
  const linked=!!hierroSync?.state?.created,enabled=pushPrefs.enabled,active=pushActive();
  const state=!enabled?'Sin activar en este dispositivo':typeof Notification!=='undefined'&&Notification.permission==='denied'?'Permiso de notificaciones retirado':!active?'Falta completar la activación':navigator.onLine===false?'Sin conexión · los avisos necesitan internet':'Notificaciones activadas';
  return `<div class="n-detail-grid n-push-grid"><section class="n-panel n-push-intro"><span class="n-eyebrow">Entrena. Nosotros llevamos el reloj.</span><h2>Tu siguiente serie,<br>sin mirar el teléfono.</h2><p>Recibe el fin del descanso aunque bloquees la pantalla o cambies de app.</p><div class="n-sync-state"><strong>${state}</strong></div><p class="hint">Programar y recibir avisos requiere internet. El entrenamiento y su registro siguen disponibles sin conexión.</p><div class="n-push-preview" aria-label="Ejemplo de notificación"><span>${uiIcon('bell')}HIERRO <small>ahora</small></span><b>Descanso listo</b><p>${pushPrefs.details?'Jalón al pecho · 65 lb × 6 reps. Placa 13 + ajuste fino 2.5 lb.':'Tu siguiente paso está listo en Hierro.'}</p><small>Ejemplo · tus avisos usarán el equipo de tu gimnasio</small></div><p data-push-status class="n-push-status" role="status">${esc(pushMessage||'Se activan solo en este dispositivo.')}</p><div class="n-sync-actions">${enabled?uiButton('Desactivar en este dispositivo','uiPushDisable()','bell','secondary'):!linked?uiButton('Vincular mi espacio','uiPushLinkSpace()','shield'):uiButton(pushEnabling?'Preparando…':'Activar notificaciones','uiPushEnable()','bell')}${active?uiButton('Probar con pantalla bloqueada','uiPushTest()','phone','text'):''}</div>${!linked?'<p class="hint">Primero crea o vincula tu espacio de sincronización. Así podemos programar los avisos sin una computadora encendida.</p>':''}</section><aside class="n-preference-panels"><section class="n-panel"><h2>Elige tus avisos.</h2><p class="hint">${active?'Estas preferencias se aplican a los avisos de este dispositivo.':'Preferencias preparadas. Los avisos comenzarán después de activarlos.'}</p>${uiToggle('Descanso terminado','Incluye los descansos de calentamiento.',pushPrefs.rest,"uiPushPreference('rest')")}${uiToggle('Cinco minutos sin registrar','Un recordatorio por pausa. Espera si tu descanso aún no termina.',pushPrefs.idle,"uiPushPreference('idle')")}${uiToggle('Carga y montaje en el aviso','Muestra ejercicio, peso, placa y ajuste fino en la pantalla bloqueada.',pushPrefs.details,"uiPushPreference('details')")}</section><section class="n-panel n-push-guide"><h2>En tu iPhone</h2><ol><li>Abre Hierro en Safari o en un navegador que permita añadirla al inicio.</li><li>Compartir → Añadir a pantalla de inicio.</li><li>Ábrela desde su icono, vincula tu espacio y activa los avisos aquí.</li></ol><p>Requiere iOS 16.4 o posterior. Si ya lo permitiste, revisa Ajustes → Notificaciones → Hierro Beta.</p><p>Para programar y recibir avisos hace falta internet. El sistema puede retrasarlos por conexión o concentración. Los pitidos de los últimos 3 segundos funcionan con la app abierta; el sonido del aviso lo decide el teléfono.</p></section></aside></div>`;
}
...
function pushSnapshot(){
  const s=db.active;if(!s?.exercises.length||s.localOnly||syncForeign||['conflict','waiting'].includes(hierroSync?.status))return null;
...
async function pushFlush(){
  ...
  try{
    while(pushDirty){
      ...
      pushStatus(db.active?.localOnly?'Sesión independiente: el reloj funciona aquí; sus avisos remotos no están programados.':events.length?'Avisos programados. Puedes bloquear la pantalla.':'Avisos activos · se programan durante tu sesión.');
    }
...
function uiPushLinkSpace(){window.__syncReturn={name:'settings',section:'notifications'};go({name:'settings',section:'sync'});}
