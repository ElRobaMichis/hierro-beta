...
resetDB();uiResetRest();db.settings.health='off';db.settings.rest='off';
{
 ...
 els['modalhost'].innerHTML=els['modalhost'].innerHTML.replace('<h2>','<h2 id="ui-dialog-title">');
 uiGymCheckResolve(geoToken,'geo-day',false,null,true);
 chk(!db.active&&els['modalhost'].innerHTML.includes('¿Entrenas hoy en «Gym de mi pareja»?'),'sin lectura del teléfono, la comprobación no se queda colgada: pasa a la pregunta que dicta el historial');
 uiGymCheckAnswer(geoToken,'geo-day',false,'');chk(db.active&&db.settings.gymId===other.id,'y «Seguir» empieza donde estabas');
 db.active=null;view={name:'home'};uiBeginSession('geo-day');geoToken=window.__gymCheck;
 els['modalhost'].innerHTML=els['modalhost'].innerHTML.replace('<h2>','<h2 id="ui-dialog-title">');
 uiGymCheckAnswer(geoToken,'geo-day',false,'');
 chk(db.active&&db.settings.gymId===other.id,'el botón de salida inmediata empieza aunque la lectura no haya llegado');
