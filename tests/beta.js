...
ok(fs.readdirSync(root).length===16&&!fs.existsSync(path.join(root,'tests'))&&!fs.existsSync(path.join(root,'sync-worker'))&&!fs.existsSync(path.join(root,'mockups')),'la web publica solo los recursos de la app, sin backend ni credenciales');
for(const name of ['sync-core','sync-engine','qr','sync','push-core','push'])ok(html.match(new RegExp('<script id="hierro-'+name+'">\\n([\\s\\S]*?)\\n</script>'))?.[1]===read(name+'.js'),'incluye de forma atómica '+name);
